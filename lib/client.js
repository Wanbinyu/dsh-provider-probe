...
function buildDiagnosticReport(result, inputModalities) {
	const lines = [
	...
	} else lines.push(`Error code: ${oneLine(result.failure.code)}`, ...result.failure.status === void 0 ? [] : [`HTTP status: ${String(result.failure.status)}`], ...result.failure.requestId === void 0 ? [] : [`Request ID: ${oneLine(result.failure.requestId)}`], `Message: ${oneLine(result.failure.message)}`);
	return `${lines.join("\n")}\n`;
}
...
function ProviderProbeSection({ catalog, probe, t }) {
	...
	const copyDiagnostic = async () => {
		...
		setCopyState(await (0, _deepseek_ai_dsh_client_ui_primitives.writeClipboard)(buildDiagnosticReport(result, selectedModel?.inputModalities)) ? "copied" : "failed");
	};
