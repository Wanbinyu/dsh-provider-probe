...
export function buildDiagnosticReport(result, inputModalities) {
    const lines = [
    ...
    if (result.status === 'success') {
        ...
        lines.push(`Error code: ${oneLine(result.failure.code)}`, ...result.failure.status === undefined ? [] : [`HTTP status: ${String(result.failure.status)}`], ...result.failure.requestId === undefined ? [] : [`Request ID: ${oneLine(result.failure.requestId)}`], `Message: ${oneLine(result.failure.message)}`);
    }
    return `${lines.join('\n')}\n`;
}
