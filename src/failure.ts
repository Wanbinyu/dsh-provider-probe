...
const CODE_GROUPS: ReadonlyArray<readonly [ProbeFailureCategory, ReadonlySet<string>]> = [
...
} satisfies Record<ProbeFailureCategory, string>
...
export function classifyProbeFailure(failure: FailureEvidence): ProbeFailureCategory {
  ...
  if (/\bpermission denied\b|\baccess denied\b|\bforbidden\b/.test(message)) return 'permission'
  ...
  if (/\btimed? out\b|\btimeout\b|\bdeadline exceeded\b/.test(message)) return 'timeout'
